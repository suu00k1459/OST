...
import numpy as np  # currently unused but kept in case you add weight ops later
...
# ---------------------------------------------------------------------
# PATH HANDLING (robust for Docker and local)
# ---------------------------------------------------------------------
# This file lives in /app/scripts inside the container.
# We want models under /app/models regardless of current working dir.
SCRIPT_DIR = Path(__file__).resolve().parent          # .../scripts
PROJECT_ROOT = SCRIPT_DIR.parent                      # .../ (=/app in Docker)
...
class ModelMetadata:
    ...
    def __init__(self, model_type: str, version: int):
        ...
        self.num_devices_aggregated: int = 0
        self.tags: List[str] = []
...
class GlobalModel:
    ...
    def __init__(self, version: int):
        ...
        self.weights: Optional[np.ndarray] = None
        self.parameters: Dict[str, Any] = {}
        self.device_contributions: Dict[str, Any] = {}  # Track which devices contributed
...
class ModelManager:
    ...
    def __init__(self):
        self.local_models_registry: Dict[str, int] = {}  # device_id -> latest_version
        self.global_models_registry: Dict[int, Path] = {}  # version -> path
        ...
        try:
            ...
            for model_file in LOCAL_MODELS_DIR.glob("device_*.pkl"):
                ...
                if len(parts) >= 3:
                    ...
                    if (
                        device_id not in self.local_models_registry
                        or version > self.local_models_registry[device_id]
                    ):
    ...
    # ------------------------------------------------------------------
    def get_local_model_info(self, device_id: str) -> Optional[Dict[str, Any]]:
    ...
    def get_global_model_info(self, version: Optional[int] = None) -> Optional[Dict[str, Any]]:
    ...
    def list_local_models(self, device_id: Optional[str] = None) -> List[Dict[str, Any]]:
        """List all local models (optionally filtered by device)"""
    ...
    def list_global_models(self) -> List[Dict[str, Any]]:
    ...
    def cleanup_old_models(self, keep_local: int = 5, keep_global: int = 10) -> None:
        ...
        for device_id in list(self.local_models_registry.keys()):
...
def main():
    """Demo model manager usage (for local testing)"""
    logger.info("=" * 70)
    ...
    for model in local_models:
        logger.info(
            f"  {model['device_id']} v{model['version']}: "
